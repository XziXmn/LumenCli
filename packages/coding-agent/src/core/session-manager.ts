...
export interface AppendCompactionOptions<T = unknown> {
	summaryPlacement?: "before-kept" | "after-kept";
	replacementMessages?: AgentMessage[];
	details?: T;
	fromHook?: boolean;
...
export function buildSessionContext(
	...
	if (compaction) {
		...
		if (compaction.replacementMessages && compaction.replacementMessages.length > 0) {
		...
		const summaryPlacement = compaction.summaryPlacement ?? "before-kept";

		if (summaryPlacement === "before-kept") {
		...
		if (summaryPlacement === "after-kept") {
			messages.push(summaryMessage);
		}
 ...
 * Encodes cwd into a safe directory name under ~/.lumen/agent/sessions/.
 */
...
export class SessionManager {
	...
	appendCompaction<T = unknown>(
		...
		firstKeptEntryId: string,
		tokensBefore: number,
		options?: AppendCompactionOptions<T>,
	): string {
		const entry: CompactionEntry<T> = {
			...
			replacementMessages: options?.replacementMessages,
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.lumen/agent/sessions/<encoded-cwd>/).
	 */
