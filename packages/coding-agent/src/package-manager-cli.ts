...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|lumen] [--self] [--extensions] [--extension <source>] [--force]`;
		case "list":
...
Options:
  -l, --local    Install project-locally (.lumen/settings.json)
...
Options:
  -l, --local    Remove from project settings (.lumen/settings.json)
...
Update lumen and installed packages.

Options:
  --self                  Update lumen only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Reinstall lumen even if the current version is latest

Short forms:
  ${APP_NAME} update                Update lumen and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update lumen          Update lumen only (self works as alias to lumen)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "pi" || source === "lumen";
			if (sourceIsSelf) {
