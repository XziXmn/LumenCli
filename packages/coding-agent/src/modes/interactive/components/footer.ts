...
function prioritizeStatusEntries(entries: Array<[string, string]>): Array<[string, string]> {
...
export class FooterComponent implements Component {
	...
	render(width: number): string[] {
		...
		// Tighten the cwd line for narrow terminals.
		...
		if (state.model?.reasoning) {
			...
			rightSideWithoutProvider = thinkingLevel === "off" ? modelName : `${modelName} • ${thinkingLevel}`;
		}

		// Prepend the provider only when it comfortably fits.
		let rightSide = rightSideWithoutProvider;
		if (this.footerData.getAvailableProviderCount() > 1 && state.model) {
			const withProvider = `(${state.model!.provider}) ${rightSideWithoutProvider}`;
			if (statsLeftWidth + minPadding + visibleWidth(withProvider) <= width - 6) {
