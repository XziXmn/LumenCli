...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.lumen/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.lumen/extensions/`)
- Project skills:
  - `.lumen/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.lumen/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.lumen/agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.lumen/agent/auth.json and ~/.lumen/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.lumen/agent/extensions/`, `.lumen/extensions/`, and settings.json extension sources.
...
1. Global: `~/.lumen/agent/settings.json`
2. Project: `<cwd>/.lumen/settings.json`
