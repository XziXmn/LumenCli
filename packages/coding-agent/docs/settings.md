...
| Location | Scope |
|----------|-------|
| `~/.lumen/agent/settings.json` | Global (all projects) |
| `.lumen/settings.json` | Project (current directory) |
...
```json
{ "sessionDir": ".lumen/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `LUMEN_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.lumen/agent/settings.json` resolve relative to `~/.lumen/agent`. Paths in `.lumen/settings.json` resolve relative to `.lumen`. Absolute paths and `~` are supported.
...
Project settings (`.lumen/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.lumen/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .lumen/settings.json (project)
{
