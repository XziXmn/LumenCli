...
describe("TUI background render suppression", () => {
	it("skips loader-driven redraws while background updates are suppressed", async () => {
		const terminal = new LoggingVirtualTerminal(40, 10);
		const tui = new TUI(terminal);
		tui.shouldSuppressBackgroundRenderUpdates = () => true;
		...
		assert.strictEqual(terminal.getWrites(), "", "suppressed loader animation should not trigger terminal writes");

		loader.stop();
		tui.stop();
	});

	it("still renders focused input changes while background updates are suppressed", async () => {
		const terminal = new LoggingVirtualTerminal(40, 10);
		const tui = new TUI(terminal);
		tui.shouldSuppressBackgroundRenderUpdates = () => true;
		...
		tui.requestRender();
		await terminal.flush();
		assert.strictEqual(terminal.getWrites(), "", "background redraw should stay suppressed");

		terminal.sendInput("n");
		await terminal.waitForRender();

		assert.notStrictEqual(terminal.getWrites(), "", "focused input should still redraw during suppression");
		assert.ok(terminal.getViewport().join("\n").includes("> n"), "input text should appear in the editor row");

		tui.stop();
	});

	it("blocks non-input redraws after focused input while background updates are suppressed", async () => {
		const terminal = new LoggingVirtualTerminal(40, 10);
		const tui = new TUI(terminal);
		tui.shouldSuppressBackgroundRenderUpdates = () => true;

		const status = new TestComponent();
		status.lines = ["status: idle"];
		const input = new Input();
		tui.addChild(status);
		tui.addChild(input);
		tui.setFocus(input);
		...
		status.lines = ["status: streaming update"];
		tui.requestRender();
		await terminal.flush();
		...
		assert.strictEqual(
			terminal.getWrites(),
			"",
			"non-input redraws should stay suppressed even after the editor handled input",
		);
...
describe("TUI cursor writes", () => {
	it("keeps focused input redraws within a single synchronized write", async () => {
		const terminal = new LoggingVirtualTerminal(40, 10);
		...
		const writes = terminal.getWrites();
		assert.match(writes, /^\x1b\[\?2026h[\s\S]*\x1b\[\?2026l$/, "focused input redraw should stay in one sync frame");
		assert.strictEqual(
			(writes.match(/\x1b\[\?2026h/g) ?? []).length,
			1,
			"single input redraw should not emit a second standalone cursor write",
		);
	...
	it("keeps focused editor redraws within a single synchronized write", async () => {
		const terminal = new LoggingVirtualTerminal(40, 10);
		const tui = new TUI(terminal, true);
		...
		assert.strictEqual(
			(writes.match(/\x1b\[\?2026h/g) ?? []).length,
			1,
			"editor redraw should keep content and cursor placement in one synchronized write",
		);
		assert.ok(!writes.endsWith("\x1b[?25h"), "cursor visibility should not be emitted as a second trailing write");
