...
- 会话压缩当时已经不是纯 Pi 原生形态，而是 `core` 负责切点和会话重建、项目扩展负责 Codex 风格摘要桥接的混合态。
...
这说明主结构已经进入 BottomPane 收口阶段，但旧的“状态区一块、输入区一块”心智仍残留在部分实现、测试和文档里。
...
- `core` 负责：
  ...
  - compaction entry 落盘
  - session context 重建
- 当时的 project compaction extension 负责：
  - Codex 风格结构化摘要
  - recent user bridge
...
将当前已经落地的 `bottomPane`，继续从“统一容器”推进为“统一职责、统一验证链、统一心智”的 `BottomPane` 抽象。
...
- 下沉当时仍在 project compaction extension 中的主逻辑到 core
