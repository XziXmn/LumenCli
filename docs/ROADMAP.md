...
Lumen 是一个以 Pi 为运行时底座、以中文化和本地工作流定制为特色的个人 agent runtime fork。

它当前不是一套完全独立重写的新系统，而是一个 **Pi-powered fork**：

- 运行时、会话、工具与大量基础能力继续复用 Pi 主线
- Lumen 在其上叠加自己的中文化、工具链、交互面、任务流与本地工作流习惯
- 当前目标不是把终端界面继续推成最终产品本体，而是把 Lumen 收口成更稳定的 **runtime + adapter**
- 对外目标不是“尽快摆脱 Pi”，而是“在尽量少引入上游合并冲突的前提下，把 Lumen 做成更顺手、可被其他宿主复用的个人 agent runtime”

这里还要补充一个新的产品边界：

- 当前 `TUI` 是 **临时启动界面 / 调试入口 / fallback 入口**
- 后续更具体的产品前台，例如 `Obsidian` 写作辅助，应复用 Lumen runtime，而不是继续把终端界面当最终产品主界面

这里有一个必须反复强调的硬约束：

- `.lumen/` 是主配置面
- `.lumen/` 仅作为旧配置迁移来源，不再作为运行时 fallback

`.lumen/` 不是临时脏目录，也不是单纯历史债务。它承担旧配置资产的迁移来源责任。后续任何规划、重构或目录整理，都不能以破坏 `.lumen/` 到 `.lumen/` 的迁移路径为代价。
...
- 把终端入口收口成“好用的临时启动界面”，而不是继续增长成未来最终产品界面
...
但这条原则只服务于当前终端入口稳定化，不意味着终端界面会持续承担未来所有产品形态。

### 3. 保留迁移入口，但避免迁移入口反客为主

`.lumen/` 只保留为旧资产迁移入口，新功能默认以 `.lumen/` 为主落点。迁移入口的职责是“把旧资产搬过来”，不是“继续把所有新能力设计成 Pi-first”。
...
- 新增能力都明确其落点是 `.lumen/` 主面还是 `.lumen/` 迁移来源
...
- `TUI` 继续作为临时启动界面、调试入口和 fallback 入口
- 后续在 `TUI` 上新增的工作，应优先集中在中文化、状态栏和信息结构收口，而不是持续扩张产品专属交互
...
- 本阶段新增能力优先做成 runtime 级能力，便于未来被其他宿主复用
- 不再默认把所有新能力都先做成终端交互特化功能
...
- 这部分是未来写作辅助、陪伴型助手等上层产品最可能复用的 runtime 核心
- 因此应优先保持宿主无关，而不是绑定到终端界面的产品语义上
...
- 若未来主产品重心转向 `Obsidian` 写作辅助或其他宿主，本阶段的推进应优先考虑宿主可复用性
- 不再默认把这类执行能力与终端产品体验强绑定
...
- 新功能进入主线前有清晰验证路径
- 新规划文档不会再次失控增殖
- 上游同步不会破坏 Lumen 已有能力与 `.lumen/` 兼容层
...
这意味着后续任何需要明显产品化的能力，都应优先判断它属于 runtime 层、终端入口层，还是未来宿主前台层，避免再次把所有方向都堆回 `TUI` 主线。
