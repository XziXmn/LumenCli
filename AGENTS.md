...
## Testing Lumen Interactive Mode with tmux

To test pi's TUI in a controlled terminal environment:

```bash
# Create tmux session with specific dimensions
tmux new-session -d -s lumen-test -x 80 -y 24

# Start Lumen from source
tmux send-keys -t lumen-test "cd /path/to/LumenAgent && ./lumen-test.sh" Enter

# Wait for startup, then capture output
sleep 3 && tmux capture-pane -t lumen-test -p

# Send input
tmux send-keys -t lumen-test "your prompt here" Enter

# Send special keys
tmux send-keys -t lumen-test Escape
tmux send-keys -t lumen-test C-o  # ctrl+o

# Cleanup
tmux kill-session -t lumen-test
```
